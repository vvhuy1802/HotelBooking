...
const BookingPage = ({navigation}) => {
  ...
  useEffect(() => {
    const checkBooking = async () => {
      const booking = notify.find(item => item.data.type === 'booking');
      setIsExist(booking ? true : false);
      const notifyBooking = await notify.filter(
        item => item.data.type === 'booking',
      );
      setNotifyBooking(notifyBooking);
    };
    checkBooking();
  }, [notify]);
      ...
      {!isExist ? (
        ...
        <View
          ...
          {notify
            ?.map((item, index) => {
              return (
                item.data.type === 'booking' && (
                  <View
                    key={index + item.id}
                    style={{
                      backgroundColor: colors.box,
                      borderBottomWidth:
                        notifyBooking[0]?.id === item.id ? 0 : 1,
